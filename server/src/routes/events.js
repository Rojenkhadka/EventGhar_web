...
router.post('/', authenticateToken, requireOrganizer, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO events (title, description, date, location, max_attendees, user_id, status) 
       VALUES ($1, $2, $3, $4, $5, $6, 'PENDING_APPROVAL') 
       RETURNING *`,
      [title, description, date, location, maxAttendees || null, userId]
    );
...
router.put('/:id', authenticateToken, requireOrganizer, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE events 
       SET title = $1, description = $2, date = $3, location = $4, max_attendees = $5, status = 'PENDING_APPROVAL'
       WHERE id = $6
       RETURNING *`,
      [title, description, date, location, maxAttendees || null, id]
    );
